...
object DnsPacket {
    ...
    /** QTYPE of a parsed query — last 4 bytes of the question section are TYPE(2)+CLASS(2). */
    fun qtypeOf(query: ParsedQuery): Int {
        val q = query.rawDnsQuestionSection
        if (q.size < 4) return 0
        return ((q[q.size - 4].toInt() and 0xFF) shl 8) or (q[q.size - 3].toInt() and 0xFF)
    }

    /** Returns a copy of a raw DNS message with its transaction ID (first 2 bytes) swapped —
     *  used to serve a [DnsCache][com.fdzaki.adshield.data.DnsCache] hit under a *different*
     *  query's transaction ID than the one the cached bytes were originally captured under. */
    fun withTransactionId(message: ByteArray, txId: ByteArray): ByteArray {
        if (message.size < 2 || txId.size != 2) return message
    ...
    /**
     * Best-effort extraction of "is this a cacheable positive answer, and if
     * so what TTL (seconds) should it be cached for". Returns null for
     * anything not safely cacheable: non-zero RCODE, zero answers, or a
     * malformed/truncated message we can't confidently parse. Only the
     * FIRST answer record's TTL is used (matches typical single-A-record
     * ad-tech/CDN lookups this cache is optimizing for); multi-answer
     * responses with wildly different TTLs are rare enough not to special-case.
     */
    fun extractCacheableTtlSeconds(message: ByteArray): Long? {
        ...
        repeat(qdCount) {
            pos = skipName(message, pos) ?: return null
            pos += 4 // QTYPE + QCLASS
        }
        // First answer record: NAME (possibly a pointer), TYPE(2) CLASS(2) TTL(4) RDLENGTH(2) RDATA.
        pos = skipName(message, pos) ?: return null
        if (pos + 10 > message.size) return null
        pos += 4 // skip TYPE + CLASS
        val ttl = ((message[pos].toLong() and 0xFF) shl 24) or
            ((message[pos + 1].toLong() and 0xFF) shl 16) or
            ((message[pos + 2].toLong() and 0xFF) shl 8) or
            (message[pos + 3].toLong() and 0xFF)
        return ttl.coerceAtLeast(0L)
    }

    /** Advances past a DNS NAME field (labels or a compression pointer), returning the new
     *  position, or null if the buffer runs out before the name terminates. */
    private fun skipName(message: ByteArray, start: Int): Int? {
        ...
        while (pos < message.size) {
            ...
            when {
                len == 0 -> return pos + 1
                (len and 0xC0) == 0xC0 -> return if (pos + 1 < message.size) pos + 2 else null
                else -> {
                    pos += 1 + len
